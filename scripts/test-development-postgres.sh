...
GRANT USAGE ON SCHEMA public TO blazn_runtime,blazn_sandbox_controller;
SQL
tar -C "$repo_root" -cf - services/control-api packages/contracts | docker run --rm -i --network "$network" -e BLAZN_DEVELOPMENT_TEST_DATABASE_URL="postgresql://blazn_runtime:$runtime_password@$postgres:5432/blazn" -e BLAZN_DEVELOPMENT_TEST_ADMIN_DATABASE_URL="postgresql://postgres:$admin_password@$postgres:5432/blazn" -e MIGRATION_DATABASE_URL="postgresql://blazn_migration:$migration_password@$postgres:5432/blazn" "$node_image" sh -euc '
