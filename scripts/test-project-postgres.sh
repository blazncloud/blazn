...
admin_password=project-ci-admin
suffix=$$
network=blazn-project-pg-$suffix
postgres=blazn-project-pg-$suffix

command -v docker >/dev/null 2>&1 || { echo "docker is required for the disposable Project PostgreSQL test" >&2; exit 1; }
...
tar -C "$repo_root" -cf - services/control-api packages/contracts | docker run --rm -i --network "$network" \
  -e BLAZN_PROJECT_TEST_DATABASE_URL="postgresql://blazn_runtime:$runtime_password@$postgres:5432/blazn" \
  -e BLAZN_PROJECT_TEST_ADMIN_DATABASE_URL="postgresql://postgres:$admin_password@$postgres:5432/blazn" \
  ...
  "$node_image" sh -euc '
    ...
    node --test dist/project-store.integration.test.js
  '
