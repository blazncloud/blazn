...
admin_password=agent-harness-ci-admin
suffix=$$
network=blazn-agent-harness-pg-$suffix
postgres=blazn-agent-harness-pg-$suffix

command -v docker >/dev/null 2>&1 || { echo "docker is required for the disposable Agent/Harness PostgreSQL test" >&2; exit 1; }
...
GRANT CONNECT ON DATABASE blazn TO blazn_runtime, blazn_development_controller;
...
tar -C "$repo_root" -cf - services/control-api packages/contracts | docker run --rm -i --network "$network" \
  -e BLAZN_AGENT_HARNESS_TEST_DATABASE_URL="postgresql://blazn_runtime:$runtime_password@$postgres:5432/blazn" \
  -e BLAZN_AGENT_HARNESS_TEST_ADMIN_DATABASE_URL="postgresql://postgres:$admin_password@$postgres:5432/blazn" \
  ...
  -e BLAZN_AGENT_RUN_CONTROLLER_TEST_DATABASE_URL="postgresql://blazn_development_controller:$controller_password@$postgres:5432/blazn" \
  ...
  "$node_image" sh -euc '
    ...
    node --test dist/agent-harness-store.integration.test.js dist/agent-run-controller-store.integration.test.js
  '
