...
JSON Schema validates the closed manifest shape and rejects `.` and `..` path segments, but it cannot express uniqueness by one property across distinct array objects or compare a create request with a selected persisted template. Migration `009` therefore normalizes every version variant field (including command and resources), repository field, and artifact field into child identity tables. Deferred constraint triggers compare every normalized value to the immutable spec. Sandbox source and artifact children carry composite foreign keys to the exact selected sandbox version, and bidirectional set-difference checks require exactly one known source commit per repository plus every exact artifact-contract entry. The schema extension documents this division; it is not presented as schema-only enforcement.

Runtime cannot insert immutable versions or sandboxes directly. The owner/administrator-authorized publish function accepts application-validated RFC 8785 bytes, verifies that they decode to the exact draft spec, verifies their SHA-256 inside PostgreSQL, and derives all normalized rows from that spec. The owner/administrator/operator-authorized create function derives the selected template and variant, verifies exact sources, verifies canonical artifact-contract bytes and SHA-256 inside PostgreSQL, and derives the sandbox children. PostgreSQL deliberately does not claim to implement RFC 8785; the generated Go canonicalizer owns that validation, while the trusted database functions prove byte digest and semantic equality before persistence.
...
The operation resource includes its typed terminal receipt. Nonterminal operations require a null receipt and completion time; terminal operations require a matching receipt status and type. Successful create receipts prove a live backend UID/resource version, while successful stop/delete receipts prove artifact export, grant revocation, cleanup, and backend destruction. Sandbox-wide SSE events carry a stable event ID, sandbox-global sequence, optional operation ID, typed event name, safe payload, and timestamp. Artifact list/get responses expose immutable export receipts and same-origin download metadata; raw content uses `application/octet-stream` with exact size and SHA-256 headers.
