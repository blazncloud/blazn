...
export async function startNodeBroker(issuer?:WorkerCredentialIssuer):Promise<Server>{const resolvedIssuer=issuer??await UnixMicroK8sWorkerCredentialIssuer.connect(defaultMicroK8sIssuerSocket);const root=process.env.BLAZN_NODE_BROKER_SECRETS_ROOT??"/etc/blazn/node-broker/secrets",databaseUrl=(await readFile(`${root}/database-url`,"utf8")).trim();if(!databaseUrl)throw new Error("Node broker database URL is empty");const port=Number(process.env.NODE_BROKER_PORT??"8081");if(!Number.isSafeInteger(port)||port<1||port>65535)throw new Error("Node broker port is invalid");const database=createDatabase(databaseUrl);try{await database.query("SELECT 1");const startupKey=await readJoinCredentialKey(`${root}/join-credential-v1`);startupKey.fill(0);}catch(error){await database.end();throw error;}const service=new NodeBrokerService(new PgNodeBrokerStore(database),()=>readJoinCredentialKey(`${root}/join-credential-v1`),resolvedIssuer),server=createNodeBrokerServer(service);await new Promise<void>((resolve,reject)=>{server.once("error",reject);server.listen(port,"127.0.0.1",resolve);});server.once("close",()=>void database.end());return server;}
