...
test("controller finalization verifies committed inputs, tenant resolution, evidence, reproducibility, and publication bindings", async () => {
  ...
  const cases: Array<[string, (value: Record<string, unknown>) => void]> = [
    ...
    ["substituted valid Artifact digest", (value) => { ((value.finalization as { artifacts: Array<Record<string, unknown>> }).artifacts[1]!).contentDigest = `sha256:${"f".repeat(64)}`; }],
    ["substituted manifest media", (value) => { ((value.evidence as { artifactManifest: Array<Record<string, unknown>> }).artifactManifest[1]!).mediaType = "document"; }],
    ["cross-role digest reuse", (value) => { const manifest=(value.evidence as { artifactManifest: Array<Record<string, unknown>> }).artifactManifest,artifacts=(value.finalization as { artifacts: Array<Record<string, unknown>> }).artifacts;manifest[1]!.contentDigest=manifest[0]!.contentDigest;artifacts[1]!.contentDigest=artifacts[0]!.contentDigest; }],
    ["refresh content mismatch", (value) => { (((value.outputs as { refreshArtifacts: Record<string, Record<string, unknown>> }).refreshArtifacts)["linux/amd64"]!).contentDigest = `sha256:${"f".repeat(64)}`; }],
