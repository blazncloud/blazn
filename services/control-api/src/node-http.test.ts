...
test("join consumption requires authoritative quarantined worker observation",async()=>{let observed="",binding:unknown;const service={async consumeJoin(_issuance:string,key:string){observed=key;return{id:nodeId};}} as unknown as NodeService,broker={async observe(_id:string,value:unknown){binding=value;},async issue(){throw new Error();},async health(){}};const server=nodeServer(new NodeHttpRouter(service,broker));await listen(server);const body={nodeId,enrollmentId,planId:"55555555-5555-4555-8555-555555555555",joinedNodeUid:"uid-a",joinedNodeName:"node-a",resourceVersion:"1",clusterId:"cluster-a"};const url=origin(server)+`/v1/node-service/join-credentials/66666666-6666-4666-8666-666666666666/consume`;try{const missing=await fetch(url,{method:"POST",headers:{"content-type":"application/json","x-blazn-node-proof":"x".repeat(86)},body:JSON.stringify(body)});assert.equal(missing.status,400);const accepted=await fetch(url,{method:"POST",headers:{"content-type":"application/json","x-blazn-node-proof":"x".repeat(86),"idempotency-key":"consume-http-key"},body:JSON.stringify(body)});assert.equal(accepted.status,200);assert.equal(observed,"consume-http-key");assert.deepEqual(binding,{clusterId:"cluster-a",nodeName:"node-a",nodeUid:"uid-a",resourceVersion:"1"});}finally{await close(server);}});

test("activation route requires and threads proof and idempotency",async()=>{let key="",proof="";const service={async activate(_body:unknown,k:string,p:string){key=k;proof=p;return{id:nodeId};}} as unknown as NodeService,server=nodeServer(new NodeHttpRouter(service));await listen(server);const body={expectedVersion:2,receipt:{schemaVersion:"nodes/v1alpha1"},kubernetesBinding:{clusterId:"cluster-a",nodeName:"worker-a",nodeUid:"uid-a",resourceVersion:"7"}};try{const missing=await fetch(origin(server)+"/v1/node-service/activations",{method:"POST",headers:{"content-type":"application/json","x-blazn-node-proof":"x".repeat(86)},body:JSON.stringify(body)});assert.equal(missing.status,400);const accepted=await fetch(origin(server)+"/v1/node-service/activations",{method:"POST",headers:{"content-type":"application/json","x-blazn-node-proof":"x".repeat(86),"idempotency-key":"activate-http-key"},body:JSON.stringify(body)});assert.equal(accepted.status,200);assert.equal(key,"activate-http-key");assert.equal(proof,"x".repeat(86));}finally{await close(server);}});

test("recognized Node routes reject unsupported methods before identifiers",async()=>{const server=nodeServer(new NodeHttpRouter({} as NodeService));await listen(server);try{for(const path of [`/v1/workspaces/${workspaceId}/node-enrollments`,`/v1/node-enrollments/${enrollmentId}/exchange`,`/v1/workspaces/${workspaceId}/nodes`,`/v1/nodes/${nodeId}`,`/v1/nodes/${nodeId}/operations`,`/v1/nodes/${nodeId}/events`,"/v1/node-service/heartbeats","/v1/node-service/activations",`/v1/node-service/join-credentials/${enrollmentId}/consume`]){const response=await fetch(origin(server)+path,{method:"OPTIONS"});assert.equal(response.status,405,path);assert.equal((await response.json() as {code:string}).code,"method_not_allowed");}}finally{await close(server);}});
