...
function token(overrides: Record<string, unknown> = {}): string {
  ...
  const payload = Buffer.from(JSON.stringify({ iss: "https://identity.blazn.example/", aud: "client-id", sub: "zitadel-user-id", email: "USER@example.com", email_verified: true, name: "Blaze User", nonce: "nonce", amr: ["pwd", "mfa"], iat: now - 5, exp: now + 300, ...overrides })).toString("base64url");
...
test("verified OIDC identity requires signature, issuer, audience, nonce, email, and MFA", () => {
  assert.deepEqual(verifyOidcIdToken(token(), verification), { issuer: verification.issuer, subject: "zitadel-user-id", email: "user@example.com", displayName: "Blaze User", amr: ["pwd", "mfa"] });
  ...
  assert.doesNotThrow(() => verifyOidcIdToken(token({ amr: ["passwordless"] }), verification));
});
