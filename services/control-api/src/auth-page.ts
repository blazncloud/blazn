...
export interface LegacyActivationPageInput {
  code: string;
  deviceName: string;
  platform: string;
}
...
export function renderLegacyActivationPage(input: LegacyActivationPageInput): string {
  return document("Authorize Blazn", `<div class="shell"><section class="story"><div class="brand">${flame()}<span>Blazn</span></div><div class="hero"><div class="eyebrow">Your AI workforce, one command away</div><h1>Build with agents.<br>Keep control.</h1><p>Securely connect this machine to the workspace where your models, tools, environments, and team operate together.</p></div><div class="proof"><span>Device-bound sessions</span><span>Verified identity</span><span>Revocable access</span></div></section><main class="panel"><div class="card"><div class="mobile-brand">${flame()}<span>Blazn</span></div><div class="device"><strong>${escapeHtml(input.deviceName)}</strong><span>${escapeHtml(input.platform)}</span><div class="code">${escapeHtml(input.code)}</div></div><h2>Authorize this device</h2><p class="lede">Confirm the code matches your CLI, then sign in to approve this machine.</p><form method="post" action="/v1/auth/device/approve"><input type="hidden" name="user_code" value="${escapeHtml(input.code)}"><label class="field">Email<input name="email" type="email" autocomplete="username" required></label><label class="field">Password<input name="password" type="password" autocomplete="current-password" required></label><button class="primary">Authorize this device</button></form><p class="terms">Approval is limited to the device and code shown above. You can revoke access from Blazn at any time.</p></div></main></div>`);
}

export function renderLegacyAuthResult(title: string, message: string, ok: boolean): string {
  return document(title, `<main class="panel"><div class="card success">${flame()}<h1>${escapeHtml(title)}</h1><p>${escapeHtml(message)}</p><p>${ok ? "You may close this window and return to the CLI." : "Return to the activation page and try again."}</p></div></main>`);
}

export function sendHtml(response: ServerResponse, status: number, html: string): void {
  response.writeHead(status, { "content-type": "text/html; charset=utf-8", "content-length": String(Buffer.byteLength(html)), "cache-control": "no-store", "content-security-policy": "default-src 'none'; style-src 'unsafe-inline'; form-action 'self'; frame-ancestors 'none'; base-uri 'none'", "x-frame-options": "DENY", "referrer-policy": "no-referrer", "permissions-policy": "camera=(), microphone=(), geolocation=()" });
  response.end(html);
}
