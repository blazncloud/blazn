...
import { nodeRoleAllows, NodeHttpError, type ExchangeNodeEnrollmentResponse, type KubernetesBinding, type NodeActivationRequest, type NodeArchitecture, type NodeEvent, type NodeOperationType, type NodeOperationView, type NodePlanSigningKey, type NodePlatform, type NodePrincipal, type NodeView } from "./node-types.js";
...
export class NodeService {
  ...
  async exchangeEnrollment(enrollmentId:string,input:ExchangeEnrollmentInput):Promise<ExchangeNodeEnrollmentResponse>{
    ...
    return this.store.transaction(async(tx)=>{
      ...
      if(input.kubernetesBinding){const cluster=plan.cluster;if(!cluster||typeof cluster!=="object"||Array.isArray(cluster)||input.kubernetesBinding.clusterId!==(cluster as Record<string,unknown>).id||input.kubernetesBinding.nodeName!==plan.hostname)throw new NodeHttpError("enrollment_invalid","adopted Kubernetes binding does not match the signed install target");}
  ...
  async activate(input:NodeActivationRequest,idempotencyKey:string,proof:string):Promise<NodeView>{
    validIdempotency(idempotencyKey);if(!Number.isSafeInteger(input.expectedVersion)||input.expectedVersion<1)invalid("expectedVersion is invalid");validateBinding(input.kubernetesBinding);const receipt=validateInstallReceipt(input.receipt);const digest=nodeInstallReceiptDigest(receipt);if(receipt.digest!==digest)throw new NodeHttpError("invalid_request","install receipt digest does not match payload");const request={expectedVersion:input.expectedVersion,receipt,kubernetesBinding:input.kubernetesBinding};const activationDigest=requestDigest(request);
    return this.store.transaction(async tx=>{const authority=await tx.activationAuthority(String(receipt.nodeId));if(!authority||authority.lifecycleState==="removed"||authority.trustState==="revoked")throw new NodeHttpError("identity_rejected","node activation identity is not active");if(authority.nodeVersion!==input.expectedVersion&&authority.lifecycleState!=="active")throw new NodeHttpError("version_conflict","node version changed");if(authority.generation!==receipt.nodeIdentityGeneration||authority.planId!==receipt.planId||`sha256:${authority.planDigest}`!==receipt.planDigest||authority.nodeId!==receipt.nodeId||authority.signingKeyId!==receipt.signingKeyId||`sha256:${authority.publicKeyFingerprint}`!==receipt.signerFingerprint||!authority.kubernetesBinding||canonicalJson(authority.kubernetesBinding)!==canonicalJson(input.kubernetesBinding))throw new NodeHttpError("state_conflict","activation receipt or Kubernetes binding differs from trusted node state");if(!verifyNodeInstallReceiptSignature(authority.publicKey,digest,String(receipt.signature))||!verifyNodeProof(authority.publicKey,"blazn-node-activation-v1",request,proof))throw new NodeHttpError("identity_rejected","activation signatures could not be verified");return tx.activateNode({nodeId:authority.nodeId,expectedVersion:input.expectedVersion,idempotencyKey,requestDigest:activationDigest,receipt,receiptId:String(receipt.receiptId),receiptDigest:digest.slice(7),planId:authority.planId,identityGeneration:Number(receipt.nodeIdentityGeneration),signerFingerprint:String(receipt.signerFingerprint).slice(7),signingKeyId:String(receipt.signingKeyId),signature:String(receipt.signature),kubernetesBinding:input.kubernetesBinding});}).catch(mapStoreError);
  }
...
function validateInstallReceipt(value:Record<string,unknown>):Record<string,unknown>{const required=["schemaVersion","receiptId","planId","planDigest","nodeId","generation","nodeIdentityGeneration","signerKind","signerFingerprint","state","currentStage","owner","binary","service","mutations","residues","createdAt","updatedAt","signingKeyId","digest","signature"];exact(value,required);if(value.schemaVersion!=="nodes/v1alpha1"||value.signerKind!=="node_identity"||value.state!=="active"||value.currentStage!=="complete")invalid("install receipt terminal state is invalid");for(const field of ["receiptId","planId","nodeId"]){validUuid(String(value[field]),field);}for(const field of ["generation","nodeIdentityGeneration"]){if(!Number.isSafeInteger(value[field])||Number(value[field])<1)invalid(`install receipt ${field} is invalid`);}for(const field of ["planDigest","signerFingerprint","digest"]){requiredRenderedDigest(value[field],field);}requiredText(value.signingKeyId,"signingKeyId",128);requiredSignature(value.signature);if(!Number.isFinite(Date.parse(String(value.createdAt)))||!Number.isFinite(Date.parse(String(value.updatedAt)))||Date.parse(String(value.updatedAt))<Date.parse(String(value.createdAt)))invalid("install receipt timestamps are invalid");const owner=record(value.owner,"owner");exact(owner,["uid","pid","processStartIdentity","nonce"]);for(const field of ["uid","pid"]){if(!Number.isSafeInteger(owner[field])||Number(owner[field])<0)invalid(`owner.${field} is invalid`);}requiredText(owner.processStartIdentity,"owner.processStartIdentity",256);requiredText(owner.nonce,"owner.nonce",256);const binary=record(value.binary,"binary");exact(binary,["path","digest"]);requiredText(binary.path,"binary.path",4096);requiredRenderedDigest(binary.digest,"binary.digest");const service=record(value.service,"service");exact(service,["manager","name","definitionDigest","priorEnabled","priorActive"]);requiredText(service.manager,"service.manager",64);requiredText(service.name,"service.name",256);requiredRenderedDigest(service.definitionDigest,"service.definitionDigest");if(typeof service.priorEnabled!=="boolean"||typeof service.priorActive!=="boolean")invalid("service prior state is invalid");const mutations=array(value.mutations,"mutations",4096);for(const item of mutations){const mutation=record(item,"mutation");exact(mutation,["ordinal","kind","target","priorState","rollbackMaterial","desiredDigest","status"]);if(!Number.isSafeInteger(mutation.ordinal)||Number(mutation.ordinal)<0)invalid("mutation ordinal is invalid");for(const field of ["kind","target","priorState","status"]){requiredText(mutation[field],`mutation.${field}`,4096);}requiredRenderedDigest(mutation.desiredDigest,"mutation.desiredDigest");record(mutation.rollbackMaterial,"mutation.rollbackMaterial");}const residues=array(value.residues,"residues",4096);if(residues.length!==0)invalid("active install receipt cannot contain residues");rejectSecrets(value);return value;}
