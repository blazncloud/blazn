...
const oidcClient = config.auth0 ? new OidcClient({ issuerUrl: config.auth0.issuerUrl, clientId: config.auth0.clientId, clientSecret: config.auth0.clientSecret, callbackUrl: `${config.publicUrl}/v1/auth/oidc/callback`, requireMfa: config.auth0.requireMfa }) : undefined;
const oidcKey = config.auth0 ? oidcCookieKey(config.auth0.cookieKey) : undefined;
...
async function activationPage(response: ServerResponse, code: string, mode: AuthMode): Promise<void> {
  ...
  sendHtml(response, 200, renderActivationPage({ code: escaped, deviceName: device.device_name, platform: device.platform, mode, oidcEnabled: Boolean(oidcClient), socialConnections: config.auth0?.connections ?? [] }));
}

async function startOidc(request: IncomingMessage, response: ServerResponse, url: URL): Promise<void> {
  if (!oidcClient || !oidcKey || !config.auth0) throw new HttpError("not_found", "social identity is not configured");
  ...
  if (connection && !config.auth0.connections.includes(connection)) throw new HttpError("invalid_request", "social connection is not enabled");
  ...
  const destination = await oidcClient.authorizationUrl(transaction, connection);
