...
import type { EnrollmentRecord, ExchangeNodeEnrollmentResponse, KubernetesBinding, NodeActivationGrant, NodeEnrollmentIdentity, NodeEvent, NodeOperationType, NodeOperationView, NodePlanSigningKey, NodeView } from "./node-types.js";
...
class PgNodeTransaction implements NodeTransaction {
  ...
  async activationReplay(nodeId:string,idempotencyKey:string,requestDigestValue:string,receiptDigest:string):Promise<NodeActivationGrant|undefined>{const result=await this.client.query("SELECT request_digest,receipt_digest,activation_grant FROM node_install_receipts WHERE node_id=$1 AND activation_idempotency_key=$2 FOR UPDATE",[nodeId,idempotencyKey]);const replay=result.rows[0];if(!replay)return undefined;if(replay.request_digest.trim()!==requestDigestValue||replay.receipt_digest.trim()!==receiptDigest)throw Object.assign(new Error("idempotency key is bound to another activation"),{nodeCode:"idempotency_conflict"});if(!replay.activation_grant)throw new Error("persisted activation has no signed grant");return replay.activation_grant as NodeActivationGrant;}
  ...
  async activateNode(input:{nodeId:string;expectedVersion:number;idempotencyKey:string;requestDigest:string;receipt:Record<string,unknown>;receiptId:string;receiptDigest:string;planId:string;identityGeneration:number;signerFingerprint:string;signingKeyId:string;signature:string;kubernetesBinding:KubernetesBinding;activationGrant:NodeActivationGrant}):Promise<{node:NodeView;activationGrant:NodeActivationGrant}>{
    const replayResult=await this.client.query("SELECT request_digest,receipt_digest,activation_grant FROM node_install_receipts WHERE node_id=$1 AND activation_idempotency_key=$2",[input.nodeId,input.idempotencyKey]);const replay=replayResult.rows[0];if(replay){if(replay.request_digest.trim()!==input.requestDigest||replay.receipt_digest.trim()!==input.receiptDigest)throw Object.assign(new Error("idempotency key is bound to another activation"),{nodeCode:"idempotency_conflict"});if(!replay.activation_grant)throw new Error("persisted activation has no signed grant");const node=await this.nodeById(input.nodeId);if(!node)throw new Error("activated node disappeared");return{node,activationGrant:replay.activation_grant as NodeActivationGrant};}
  ...
  async heartbeatState(nodeId: string): Promise<HeartbeatState | undefined> {
    ...
    return r ? {identityGeneration:Number(r.identity_generation),bootId:r.boot_id,sequence:Number(r.sequence),sentAt:r.sent_at,capabilityDigest:r.capability_digest?.trim() ?? null,requestDigest:r.request_digest?.trim() ?? null}:undefined;
  }
  ...
  async recordHeartbeat(input: { nodeId:string;identityGeneration:number;bootId:string;sequence:number;sentAt:Date;capabilityDigest:string;requestDigest:string;capability:Record<string,unknown>;health:unknown;priorKubernetesResourceVersion:string;kubernetesBinding:KubernetesBinding }): Promise<void> {
    ...
    await this.client.query(`INSERT INTO node_heartbeat_state(node_id,identity_generation,boot_id,sequence,sent_at,received_at,capability_digest,health,request_digest) VALUES($1,$2,$3,$4,$5,now(),$6,$7,$8)
      ON CONFLICT(node_id) DO UPDATE SET identity_generation=EXCLUDED.identity_generation,boot_id=EXCLUDED.boot_id,sequence=EXCLUDED.sequence,sent_at=EXCLUDED.sent_at,received_at=now(),capability_digest=EXCLUDED.capability_digest,health=EXCLUDED.health,request_digest=EXCLUDED.request_digest`,[input.nodeId,input.identityGeneration,input.bootId,input.sequence,input.sentAt,input.capabilityDigest,input.health,input.requestDigest]);
