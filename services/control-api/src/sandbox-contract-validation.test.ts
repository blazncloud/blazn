...
test("OpenAPI immutable OCI reference agrees with the template boundary", async () => {
  ...
  for (const value of [`registry.example.test:80/blazn/sandbox@${digest}`, `registry.example.test:443/blazn/sandbox__image@${digest}`, `registry.example.test/blazn/sandbox--image@${digest}`]) {
    assert.equal(validate(value), true, `OpenAPI rejected ${value}`);
  }
  for (const value of [`blazn/sandbox@${digest}`, `registry.example.test:0/blazn/sandbox@${digest}`, `registry.example.test:080/blazn/sandbox@${digest}`, `registry.example.test/blazn//sandbox@${digest}`]) {
