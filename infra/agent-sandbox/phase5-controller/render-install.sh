...
printf '%s\n' "$BLAZN_PHASE5_TRANSACTION_ID" | LC_ALL=C grep -Eq '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' || fail "BLAZN_PHASE5_TRANSACTION_ID is invalid"
if [ "${#BLAZN_KUBERNETES_CLUSTER_ID}" -gt 128 ] || ! printf '%s' "$BLAZN_KUBERNETES_CLUSTER_ID" | LC_ALL=C grep -Eq '^[a-z0-9][a-z0-9._:-]*$'; then
  fail "BLAZN_KUBERNETES_CLUSTER_ID is invalid"
fi
...
valid_host_cidr "$BLAZN_ACCESS_SERVICE_CLUSTER_IP/32" || fail "BLAZN_ACCESS_SERVICE_CLUSTER_IP must be one exact, usable IPv4 address"
valid_host_cidr "$BLAZN_ACCESS_SOURCE_CIDR" || fail "BLAZN_ACCESS_SOURCE_CIDR must be one exact, usable IPv4 /32"
...
sed \
  ...
  -e "s|BLAZN_SANDBOX_IO_IMAGE_REF|$BLAZN_SANDBOX_IO_IMAGE|g" \
  ...
  -e "s|BLAZN_PHASE5_TRANSACTION_ID|$BLAZN_PHASE5_TRANSACTION_ID|g" \
  ...
  -e "s|BLAZN_KUBERNETES_CLUSTER_ID|$BLAZN_KUBERNETES_CLUSTER_ID|g" \
  -e "s|BLAZN_KUBERNETES_API_AUDIENCE|$BLAZN_KUBERNETES_API_AUDIENCE|g" \
  -e "s|BLAZN_BEN1_POSTGRES_CIDR|$BLAZN_BEN1_POSTGRES_CIDR|g" \
  -e "s|BLAZN_ACCESS_SERVICE_CLUSTER_IP|$BLAZN_ACCESS_SERVICE_CLUSTER_IP|g" \
  -e "s|BLAZN_ACCESS_SOURCE_CIDR|$BLAZN_ACCESS_SOURCE_CIDR|g" \
