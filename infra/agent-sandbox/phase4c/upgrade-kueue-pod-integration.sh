...
command -v patch >/dev/null 2>&1 || { printf 'patch is required\n' >&2; exit 1; }
[ -f "$chart" ] && [ ! -L "$chart" ] && [ "$(stat -c '%h' "$chart")" = 1 ] || { printf 'Kueue chart file is unsafe\n' >&2; exit 1; }
sealed=$(mktemp -d /tmp/blazn-kueue-sealed.XXXXXX)
chmod 0700 "$sealed"
install -o root -g root -m 0400 "$chart" "$sealed/upstream-kueue-0.14.3.tgz"
install -o root -g root -m 0400 "$ROOT/kueue-pod-config.yaml" "$sealed/controller-manager.yaml"
install -o root -g root -m 0400 "$ROOT/kueue-pod-webhook-selector.patch" "$sealed/webhook-selector.patch"
sealed_chart=$sealed/upstream-kueue-0.14.3.tgz
sealed_config=$sealed/controller-manager.yaml
sealed_patch=$sealed/webhook-selector.patch
current_manifest=''
before_workloads=''
...
[ "$(stat -c '%u:%a:%h' "$sealed_chart")" = 0:400:1 ] && [ "$(stat -c '%u:%a:%h' "$sealed_config")" = 0:400:1 ] && [ "$(stat -c '%u:%a:%h' "$sealed_patch")" = 0:400:1 ] || { printf 'sealed Kueue inputs are unsafe\n' >&2; exit 1; }
...
mkdir -m 0700 "$sealed/chart-source"
tar -xzf "$sealed_chart" -C "$sealed/chart-source" --no-same-owner --no-same-permissions
patch -s -f -d "$sealed/chart-source" -p1 <"$sealed_patch"
helm package "$sealed/chart-source/kueue" --destination "$sealed" >/dev/null
derived_chart=$sealed/kueue-0.14.3.tgz
[ -f "$derived_chart" ] && [ ! -L "$derived_chart" ] || { printf 'patched Kueue chart was not produced\n' >&2; exit 1; }
...
upgraded=true
helm upgrade kueue "$derived_chart" -n kueue-system --reuse-values \
