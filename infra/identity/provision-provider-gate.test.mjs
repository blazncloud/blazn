...
test("replaces the placeholder with an independently authorized gate token", async () => {
  ...
  await writeFile(preloadPath, `let sentinelState; globalThis.fetch = async (input, init = {}) => { const path = new URL(input).pathname; const body = init.body ? JSON.parse(init.body) : {}; const json = (value, status = 200) => new Response(JSON.stringify(value), { status, headers: { "content-type": "application/json" } }); if (path === "/v2/organizations/_search") { if (body.queries?.[0]?.idQuery) return json(sentinelState ? { details: { totalResult: "1" }, result: [{ id: "blazn-provider-gate-sentinel", name: "Blazn Provider Gate Authority Sentinel", state: sentinelState }] } : { details: { totalResult: "0" }, result: [] }); return json({ details: { totalResult: "1" }, result: [{ id: "123456789", state: "ORGANIZATION_STATE_ACTIVE" }] }); } if (path === "/v2/users/blazn-provider-gate" && init.method === "GET") return json({}, 404); if (path === "/v2/users/new") return json({ id: "blazn-provider-gate" }); if (path === "/members") return json({ details: {} }); if (path === "/v2/users/blazn-provider-gate/pats") return json({ token: "authoritative-gate-token" }); if (path === "/v2/organizations" && init.method === "POST") { sentinelState = "ORGANIZATION_STATE_ACTIVE"; return json({ organizationId: "blazn-provider-gate-sentinel" }, 201); } if (path === "/v2/organizations/blazn-provider-gate-sentinel/deactivate") { sentinelState = "ORGANIZATION_STATE_INACTIVE"; return json({}); } if (path === "/auth/v1/users/me") return json({ user: { id: "blazn-provider-gate" } }); return json({ code: "unexpected" }, 500); };\n`);
  try {
