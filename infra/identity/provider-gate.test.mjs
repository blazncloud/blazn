...
test("withdraws Login authorization when a provider becomes active after startup", async () => {
  ...
  await writeFile(preloadPath, `import { readFile } from "node:fs/promises"; globalThis.fetch = async () => new Response(JSON.stringify({ details: {}, identityProviders: JSON.parse(await readFile(process.env.MOCK_IDP_STATE_FILE, "utf8")) }), { status: 200, headers: { "content-type": "application/json" } });\n`);
