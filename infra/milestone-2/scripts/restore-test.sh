...
if ! "$SCRIPT_DIR/../../node/scripts/verify-backup-metadata.sh" "$backup/metadata.json" "$node_receipt" "$node_inventory" >/dev/null; then
  die "backup rollback inventory is invalid"
fi
