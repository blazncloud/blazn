...
for command_name in cp dirname getent groupadd install jq openssl sha256sum stat sync wc; do need "$command_name"; done
...
for path in "$SOURCE" "$ROOT" "$BINARY" "$UNIT" "$TMPFILES" "$RECEIPT" "$RECOVERY"; do case "$path" in /*) ;; *) die "all issuer paths must be absolute" ;; esac; done
...
for path in "$ROOT" "$BINARY" "$UNIT" "$TMPFILES" "$RECEIPT" "$RECOVERY"; do safe_parent "$path"; done
...
if [ "$current" = secret-created ]; then
  ...
  tmp=$RECEIPT.tmp.$$; jq --arg digest "sha256:$(sha "$ROOT/config.json")" '.config.digest=$digest' "$RECEIPT" >"$tmp"; chmod 0600 "$tmp"; sync_path "$tmp"; mv -- "$tmp" "$RECEIPT"; phase config-bound; current=config-bound; fault config-bound
fi
