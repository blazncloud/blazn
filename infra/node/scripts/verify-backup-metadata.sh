...
for file in "$metadata" "$receipt"; do [ -f "$file" ] && [ ! -L "$file" ] || die "required metadata input is unavailable or symlinked: $file"; done
[ -d "$inventory" ] && [ ! -L "$inventory" ] || die "Node broker key inventory is unavailable or symlinked"
...
for name in database-url enrollment-hmac-v1 join-credential-v1; do
  file=$inventory/$name
  [ -f "$file" ] && [ ! -L "$file" ] || die "Node broker key inventory entry is missing or symlinked: $name"
