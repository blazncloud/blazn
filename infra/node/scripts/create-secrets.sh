...
publish_file() {
  ...
  chmod "$mode" "$tmp"; sync_path "$tmp"; mv -- "$tmp" "$destination"; sync_path "$(dirname -- "$destination")"
...
if [ "$phase" = database-written ]; then publish_file "$stage/secrets/enrollment-hmac-v1" 0400 key; write_journal_phase hmac-written; phase=hmac-written; test_fault hmac-written; fi
if [ "$phase" = hmac-written ]; then publish_file "$stage/secrets/join-credential-v1" 0400 key; write_journal_phase join-written; phase=join-written; test_fault join-written; fi
