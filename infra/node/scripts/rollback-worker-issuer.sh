...
if [ "$phase" = service-stopped ]; then
  ...
  recovery=$(jq -er .recovery.root "$RECEIPT"); recovery_key=$(jq -er .secretRecoveryPath "$recovery/inventory.json"); [ "$recovery_key" = "$recovery/issuer-hmac-v1" ] && [ -f "$recovery_key" ] && [ ! -L "$recovery_key" ] && [ "$(stat -c '%u:%a' "$recovery_key")" = 0:400 ] && [ "sha256:$(sha "$recovery_key")" = "$(jq -er .secret.digest "$RECEIPT")" ] || die "receipt-bound recovery key changed before rollback"
