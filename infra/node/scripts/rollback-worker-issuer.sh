...
if [ "$TEST_MODE" != 1 ]; then
  if [ "$RECEIPT" != /var/lib/blazn/ownership/microk8s-worker-issuer.json ] || [ "$STATE_ROOT" != /var/lib/blazn/microk8s-worker-issuer ]; then die "issuer rollback paths differ from reviewed contract"; fi
...
if [ "$phase" = environment-restored ]; then main=$(jq -er .ownership.path "$RECEIPT"); backup=$(jq -er .ownership.backupPath "$RECEIPT"); prior=$(jq -er .ownership.priorDigest "$RECEIPT"); current=$(sha "$main"); if [ "sha256:$current" != "$prior" ]; then material=$(jq -cS '{binary,config,unit,tmpfiles,environment,secret,socket,microk8s,recovery,brokerUid,liveJoinBlocked}' "$RECEIPT"); material_digest=sha256:$(printf '%s' "$material"|sha256sum|awk '{print $1}'); jq -e --arg digest "$material_digest" '.microk8sIssuer=={receiptPath:"/var/lib/blazn/ownership/microk8s-worker-issuer.json",materialDigest:$digest}' "$main" >/dev/null || die "main receipt changed during rollback"; tmp=$main.tmp.$$; cp -- "$backup" "$tmp"; chmod 0600 "$tmp"; sync_path "$tmp"; mv -- "$tmp" "$main"; sync_path "$(dirname -- "$main")"; fi; advance main-restored; fi
