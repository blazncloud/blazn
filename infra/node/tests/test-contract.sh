...
for expected in \
  ...
  'entrypoint: ["/bin/sh", "/opt/blazn-node/verify-database.sh", "migration-boundary"]' \
...
for service in postgres object object-client object-init api-migrate node-migration-preflight database-role-hardening node-broker-verify api-bootstrap poc-identity-provision poc-identity-cleanup poc-identity-verify-cleanup; do
