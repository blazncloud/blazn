...
jq -cn --arg digest "$digest" --arg planDigest "$plan_digest" '{schemaVersion:"blazn.dev/control-plane-backup/v3",correlationId:"test",fencingToken:1,createdAt:"20260822T080000Z",database:"blazn",bucket:"blazn-poc",configDigest:("sha256:"+("1"*64)),controlApi:{sourceDigest:("sha256:"+("2"*64)),image:("blazn-control-api:source-"+("2"*64)),imageId:("sha256:"+("3"*64))},secretDigests:{"workspace-invitation-hmac-v1":("sha256:"+("4"*64))},nodeBrokerReceiptDigest:$digest,nodePlanReceiptDigest:$planDigest}' >"$tmp/metadata.json"
...
jq -cn --arg secret "$issuer_secret" '{binary:{},config:{},unit:{},tmpfiles:{},state:{path:"/var/lib/blazn-node-root/microk8s-worker-issuer",uid:0,mode:"0700"},environment:{},secret:{digest:$secret},socket:{},microk8s:{},recovery:{},brokerUid:65532,liveJoinBlocked:false}' >"$tmp/keys/microk8s-worker-issuer.json"
