...
for fault in initialized tree-created key-written metadata-written template-written published; do
  ...
  private_standard_padded=${private_standard}=
  private_hex=$(printf '%s' "$private_standard_padded" | openssl base64 -d -A | od -An -tx1 | tr -d ' \n')
  for encoding in "$private" "$private_standard" "$private_standard_padded" "$private_hex"; do
    [ -n "$encoding" ] || { printf 'derived plan signing key encoding was empty: %s\n' "$fault" >&2; exit 1; }
