...
var rootCommands = []helpCommand{
	...
	{Name: "run", Summary: "Create, watch, cancel, and retrieve Project Runs"},
...
func (a *App) writeHelp(format OutputFormat, topic string) int {
	...
	case "dev":
		output = helpOutput{Command: "dev", Usage: "blazn dev validate|register|build|status|session [options]", Summary: "Operate Development builds and bounded interactive sessions.", Commands: []helpCommand{{Name: "validate", Summary: "Validate a DevelopmentProject offline"}, {Name: "register", Summary: "Register blazn.yaml for the selected Project"}, {Name: "build", Summary: "Request an immutable multi-architecture Build"}, {Name: "status", Summary: "Get Build status"}, {Name: "session", Summary: "Start and operate a bounded development Sandbox"}}}
	case "dev-session":
		output = helpOutput{Command: "dev session", Usage: "blazn dev session start|status|exec|upload|download|patch|finish [options]", Summary: "Operate a bounded interactive development Sandbox.", Commands: []helpCommand{{Name: "start", Summary: "Start or resume a session from a clean pushed commit"}, {Name: "status", Summary: "Inspect the exact recorded Sandbox"}, {Name: "exec", Summary: "Execute a command in the session"}, {Name: "upload", Summary: "Upload one bounded file"}, {Name: "download", Summary: "Download one verified file"}, {Name: "patch", Summary: "Export a verified Git patch"}, {Name: "finish", Summary: "Export or discard changes, then prove deletion"}}}
	case "uninstall":
	...
	case "run":
		output = helpOutput{Command: "run", Usage: "blazn run create|list|get|watch|logs|cancel|result|artifacts|messages|send|claim|deliver|synthetic-progress|synthetic-complete [RUN] [options]", Summary: "Create, observe, steer, and retrieve Project Runs.", Commands: []helpCommand{{Name: "create", Summary: "Create a Run from a plan digest"}, {Name: "list", Summary: "List Runs in the selected Project"}, {Name: "get", Summary: "Get one Run"}, {Name: "watch", Summary: "Follow ordered Run events until the Run is terminal"}, {Name: "logs", Summary: "List recorded Run progress entries"}, {Name: "cancel", Summary: "Request Run cancellation"}, {Name: "result", Summary: "Show the terminal Run receipt"}, {Name: "artifacts", Summary: "List Artifacts exported by a Run"}, {Name: "messages", Summary: "List ordered messages for a Run"}, {Name: "send", Summary: "Queue an initial prompt, follow-up, or steering message"}, {Name: "claim", Summary: "Lease the next queued message for the Run executor"}, {Name: "deliver", Summary: "Acknowledge a claimed message after delivery to the agent"}, {Name: "synthetic-progress", Summary: "Record deterministic fixture progress; never live execution proof"}, {Name: "synthetic-complete", Summary: "Complete a deterministic fixture Run; never live execution proof"}}}
	case "proxy":
