...
func (a *App) writeHelp(format OutputFormat, topic string) int {
	...
	case "run":
		output = helpOutput{Command: "run", Usage: "blazn run messages|send RUN [options]", Summary: "Inspect and queue user messages for a Project Run.", Commands: []helpCommand{{Name: "messages", Summary: "List ordered messages for a Run"}, {Name: "send", Summary: "Queue an initial prompt, follow-up, or steering message"}}}
	case "proxy":
