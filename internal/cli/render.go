...
var rootCommands = []helpCommand{
	...
	{Name: "run", Summary: "List, send, queue, and steer Run messages"},
...
func (a *App) writeHelp(format OutputFormat, topic string) int {
	...
	case "run":
		output = helpOutput{Command: "run", Usage: "blazn run messages|send|claim|deliver RUN [options]", Summary: "Inspect, queue, claim, and acknowledge user messages for a Project Run.", Commands: []helpCommand{{Name: "messages", Summary: "List ordered messages for a Run"}, {Name: "send", Summary: "Queue an initial prompt, follow-up, or steering message"}, {Name: "claim", Summary: "Lease the next queued message for the Run executor"}, {Name: "deliver", Summary: "Acknowledge a claimed message after delivery to the agent"}}}
	case "proxy":
