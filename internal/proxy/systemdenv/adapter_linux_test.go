...
func (f *fakeTransport) SetEnvironment(ctx context.Context, request busRequest, expected managerProof, values []string) (managerProof, error) {
	...
	defer f.mu.Unlock()
	if expected != f.proof {
		return managerProof{}, errors.New("manager proof changed before set")
	}
	setCount := 0
...
func (f *fakeTransport) UnsetEnvironment(ctx context.Context, request busRequest, expected managerProof, names []string) (managerProof, error) {
	...
	defer f.mu.Unlock()
	if expected != f.proof {
		return managerProof{}, errors.New("manager proof changed before unset")
	}
	if f.unsetErr != nil {
...
func TestPinnedManagerRestartIsRejectedBeforePublishMutation(t *testing.T) {
	const uid = 1813
	transport := &fakeTransport{proof: proof(uid), env: map[string]string{}}
	adapter := newTestAdapter(uid, transport)
	if _, _, err := adapter.ResolveMode("session"); err != nil {
	...
	transport.proof.ManagerOwner = ":1.99"
	err := adapter.Publish(context.Background(), publicationMechanism, publishedValues())
	if !errors.Is(err, activation.ErrUnavailable) {
		t.Fatalf("restart publish error=%v", err)
	}
	if len(transport.env) != 0 {
		t.Fatalf("replacement manager was mutated: %#v", transport.env)
	}
}
...
func TestAdapterTimeoutAndCancellationAreBoundedAndTyped(t *testing.T) {
	...
	t.Run("snapshot timeout", func(t *testing.T) {
		...
		adapter.timeout = 10 * time.Millisecond
		_, err := adapter.Snapshot(context.Background(), state.EnvironmentNames[:])
		if !errors.Is(err, activation.ErrUnavailable) || !errors.Is(err, context.DeadlineExceeded) {
			t.Fatalf("error=%v", err)
		}
	})
}
