...
type transport interface {
	...
	SetEnvironment(context.Context, busRequest, managerProof, []string) (managerProof, error)
	UnsetEnvironment(context.Context, busRequest, managerProof, []string) (managerProof, error)
}
...
func (a *Adapter) Snapshot(ctx context.Context, names []string) (map[string]activation.PriorValue, error) {
	...
	if a.acceptProof(proof) != nil {
		return nil, unavailable(ctx, nil)
	}
...
func (a *Adapter) CompareAndSet(ctx context.Context, request state.CompareAndSetRequest) (state.CompareAndSetResult, error) {
	...
	proof, entries, err := a.get(ctx)
	if err != nil {
		return state.CASConflict, unavailable(ctx, err)
	}
	if a.acceptProof(proof) != nil {
		return state.CASConflict, unavailable(ctx, nil)
	}
	values, err := parseEnvironment(entries)
	if err != nil {
		return state.CASConflict, activation.ErrUnavailable
...
func (a *Adapter) probe(ctx context.Context) (managerProof, error) {
	...
	if callCtx.Err() != nil {
		return managerProof{}, errors.Join(errTransport, callCtx.Err())
	}
...
func (a *Adapter) get(ctx context.Context) (managerProof, []string, error) {
	...
	if callCtx.Err() != nil {
		return managerProof{}, nil, errors.Join(errTransport, callCtx.Err())
	}
	if err != nil {
...
func (a *Adapter) set(ctx context.Context, values []string) (managerProof, error) {
	expected, ok := a.currentProof()
	if !ok {
		return managerProof{}, errTransport
	}
	callCtx, cancel := a.callContext(ctx)
	defer cancel()
	proof, err := a.transport.SetEnvironment(callCtx, a.request(), expected, append([]string(nil), values...))
	if callCtx.Err() != nil {
		return managerProof{}, errors.Join(errTransport, callCtx.Err())
	}
	if err != nil {
...
func (a *Adapter) unset(ctx context.Context, names []string) (managerProof, error) {
	...
	callCtx, cancel := a.callContext(ctx)
	defer cancel()
	proof, err := a.transport.UnsetEnvironment(callCtx, a.request(), expected, append([]string(nil), names...))
	if callCtx.Err() != nil {
		return managerProof{}, errors.Join(errTransport, callCtx.Err())
	}
	if err != nil {
...
func (a *Adapter) currentProof() (managerProof, bool) {
	if a == nil {
		return managerProof{}, false
	}
	a.mu.Lock()
	defer a.mu.Unlock()
	return a.proof, a.pinned
}
...
func unavailable(ctx context.Context, cause error) error {
	...
	if errors.Is(cause, context.Canceled) {
		return errors.Join(activation.ErrUnavailable, context.Canceled)
	}
	if errors.Is(cause, context.DeadlineExceeded) {
		return errors.Join(activation.ErrUnavailable, context.DeadlineExceeded)
	}
...
func (dbusTransport) Probe(ctx context.Context, request busRequest) (managerProof, error) {
	proof, _, err := callManager(ctx, request, nil, "", nil)
...
func (dbusTransport) GetEnvironment(ctx context.Context, request busRequest) (managerProof, []string, error) {
	return callManager(ctx, request, nil, managerProperties+".Get", []any{managerInterface, "Environment"})
}

func (dbusTransport) SetEnvironment(ctx context.Context, request busRequest, expected managerProof, values []string) (managerProof, error) {
	proof, _, err := callManager(ctx, request, &expected, managerInterface+".SetEnvironment", []any{values})
	return proof, err
}

func (dbusTransport) UnsetEnvironment(ctx context.Context, request busRequest, expected managerProof, names []string) (managerProof, error) {
	proof, _, err := callManager(ctx, request, &expected, managerInterface+".UnsetEnvironment", []any{names})
	return proof, err
}

func callManager(ctx context.Context, request busRequest, expected *managerProof, method string, arguments []any) (managerProof, []string, error) {
	...
	if expected != nil && (expected.OwnerUID != proof.OwnerUID || expected.BusID != proof.BusID || expected.ManagerOwner != proof.ManagerOwner) {
		return managerProof{}, nil, errTransport
	}
	if method == "" {
