...
func (u *brokerArtifactUpload) write(payload []byte) error { if len(payload)==0||u.frames>=brokerMaxUploadFrames||u.received+int64(len(payload))>u.expected{return io.ErrShortWrite};u.frames++;written,err:=u.file.Write(payload);if err!=nil{return err};if written!=len(payload){return io.ErrShortWrite};if _,err:=u.hasher.Write(payload);err!=nil{return err};u.received+=int64(len(payload));return nil }
func (u *brokerArtifactUpload) complete(ctx context.Context)(client.ArtifactEnvelope,*brokerError){ defer u.abort();if u.received!=u.expected||"sha256:"+hex.EncodeToString(u.hasher.Sum(nil))!=u.digest{return client.ArtifactEnvelope{},brokerMethodFailure("artifact_digest_mismatch","Artifact bytes do not match declared size and digest",false)};value,err:=u.finish(ctx,u.file);if err!=nil{return client.ArtifactEnvelope{},mapBrokerError(err)};if value.Artifact.ID==""||value.Artifact.WorkspaceID!=u.workspaceID||value.Artifact.ProjectID!=u.projectID||value.Artifact.SourceRunID!=u.runID||value.Artifact.Name!=u.name||value.Artifact.Kind!=u.kind||value.Artifact.MediaType!=u.mediaType||value.Artifact.Status!=client.ArtifactStatusReady||!value.Artifact.DownloadAvailable||value.Artifact.Digest!=u.digest||value.Artifact.SizeBytes==nil||*value.Artifact.SizeBytes!=u.expected{return client.ArtifactEnvelope{},brokerMethodFailure("broker_response_invalid","root API returned an invalid scoped Artifact",false)};return value,nil }
