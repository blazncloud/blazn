...
type fakeBrokerAPI struct {
	...
	profile       client.ProjectProfileEnvelope
...
func TestAuthenticatedBrokerDescriptionAdvertisesOnlyImplementedCapabilities(t *testing.T) {
	...
	if failure != nil || schema != resultBrokerDescription || !ok || strings.Join(description.AvailableCapabilities, ",") != "artifact.read,artifact.write,broker.describe,project.profile.read,project.profile.write,project.read,run.cancel,run.create,run.read,run.synthetic.execute" {
