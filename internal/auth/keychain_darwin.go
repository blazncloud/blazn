...
func storeDarwinCredential(secret []byte) error {
	...
	var item uintptr
	status := add(0, uint32(len(service)), stringPointer(service), uint32(len(account)), stringPointer(account), uint32(len(secret)), bytesPointer(secret), &item)
	if item != 0 {
		release(item)
	}
	...
	item = 0
	status = find(0, uint32(len(service)), stringPointer(service), uint32(len(account)), stringPointer(account), 0, 0, &item)
