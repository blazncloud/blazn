...
func TestProjectClientRejectsInvalidInputsBeforeNetwork(t *testing.T) {
	...
	if _, err := api.PutProjectProfile(ctx, "token", projectTestWorkspaceID, projectTestProjectID, "content", "profile-put-1", PutProjectProfileRequest{}); err == nil {
		t.Fatal("invalid Project profile passed")
	}
	if requests != 0 {
...
func TestProjectProfileClientRoutesAndVersionZeroCreate(t *testing.T) {
	requests := 0
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests++
		...
		switch requests {
		case 1:
			if r.Method != http.MethodGet || r.URL.Path != "/v1/workspaces/"+projectTestWorkspaceID+"/projects/"+projectTestProjectID+"/profiles/content" {
				t.Fatalf("get %s %s", r.Method, r.URL.Path)
			}
			_, _ = w.Write([]byte(projectProfileEnvelopeJSON(1)))
		case 2:
			if r.Method != http.MethodPut || r.Header.Get("Idempotency-Key") != "profile-put-1" {
				t.Fatalf("put %s headers=%v", r.Method, r.Header)
			}
			var request PutProjectProfileRequest
			if json.NewDecoder(r.Body).Decode(&request) != nil || request.ExpectedVersion != 0 || request.Status != ProjectProfileStatusActive {
	...
	defer server.Close()
	api, _ := New(server.URL, server.Client())
	profile, err := api.GetProjectProfile(context.Background(), "access-token", projectTestWorkspaceID, projectTestProjectID, "content")
	if err != nil || profile.Profile.Kind != "content" {
		t.Fatalf("profile=%#v err=%v", profile, err)
	}
	request := PutProjectProfileRequest{SchemaVersion: "blazn.content/project/v1alpha1", DraftID: "00000000-0000-4000-8000-000000000004", ArtifactID: "00000000-0000-4000-8000-000000000005", Digest: "sha256:" + strings.Repeat("a", 64), Status: ProjectProfileStatusActive, ExpectedVersion: 0}
	if _, err := api.PutProjectProfile(context.Background(), "access-token", projectTestWorkspaceID, projectTestProjectID, "content", "profile-put-1", request); err != nil {
		t.Fatal(err)
	}
}

func projectProfileEnvelopeJSON(version int) string {
	return `{"profile":{"workspaceId":"` + projectTestWorkspaceID + `","projectId":"` + projectTestProjectID + `","kind":"content","schemaVersion":"blazn.content/project/v1alpha1","version":` + strconv.Itoa(version) + `,"draftId":"00000000-0000-4000-8000-000000000004","artifactId":"00000000-0000-4000-8000-000000000005","digest":"sha256:` + strings.Repeat("a", 64) + `","status":"active","createdBy":"00000000-0000-4000-8000-000000000003","updatedBy":"00000000-0000-4000-8000-000000000003","createdAt":"2026-08-23T00:00:00Z","updatedAt":"2026-08-23T00:00:00Z"}}`
}
