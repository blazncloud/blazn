...
const (
	ProjectProfileStatusActive   ProjectProfileStatus = "active"
	ProjectProfileStatusArchived ProjectProfileStatus = "archived"
)
...
type ProjectProfile struct {
	...
	Status        ProjectProfileStatus `json:"status"`
	CreatedBy     string               `json:"createdBy"`
	UpdatedBy     string               `json:"updatedBy"`
	CreatedAt     string               `json:"createdAt"`
	UpdatedAt     string               `json:"updatedAt"`
}
type ProjectProfileEnvelope struct {
	Profile ProjectProfile `json:"profile"`
}
type PutProjectProfileRequest struct {
	SchemaVersion   string               `json:"schemaVersion"`
	DraftID         string               `json:"draftId"`
	ArtifactID      string               `json:"artifactId"`
	Digest          string               `json:"digest"`
...
var projectProfileSchema = regexp.MustCompile(`^[A-Za-z0-9][A-Za-z0-9._/-]{0,127}$`)
var projectProfileDigest = regexp.MustCompile(`^sha256:[0-9a-f]{64}$`)
...
func (c *Client) GetProjectProfile(ctx context.Context, accessToken, workspaceID, projectID, kind string) (ProjectProfileEnvelope, error) {
	var output ProjectProfileEnvelope
	path, err := projectProfilePath(workspaceID, projectID, kind)
	if err != nil {
		return output, err
	}
	err = c.workspaceDo(ctx, http.MethodGet, path, accessToken, "", nil, nil, &output, http.StatusOK)
...
func (c *Client) PutProjectProfile(ctx context.Context, accessToken, workspaceID, projectID, kind, idempotencyKey string, request PutProjectProfileRequest) (ProjectProfileEnvelope, error) {
	var output ProjectProfileEnvelope
	path, err := projectProfilePath(workspaceID, projectID, kind)
	if err != nil {
		return output, err
	}
	if !projectProfileSchema.MatchString(request.SchemaVersion) || !projectUUID.MatchString(request.DraftID) || !projectUUID.MatchString(request.ArtifactID) || !projectProfileDigest.MatchString(request.Digest) || (request.Status != ProjectProfileStatusActive && request.Status != ProjectProfileStatusArchived) || request.ExpectedVersion < 0 {
		return output, fmt.Errorf("Project profile request is invalid")
	}
	err = c.workspaceDo(ctx, http.MethodPut, path, accessToken, idempotencyKey, nil, request, &output, http.StatusOK)
	return output, err
}
...
func projectProfilePath(workspaceID, projectID, kind string) (string, error) {
	resource, err := projectResourcePath(workspaceID, projectID)
	if err != nil {
		return "", err
	}
	if !projectKind.MatchString(kind) {
		return "", fmt.Errorf("Project profile kind is invalid")
	}
	return resource + "/profiles/" + url.PathEscape(kind), nil
}
